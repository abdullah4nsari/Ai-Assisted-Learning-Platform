        ...
        if(paragraphWordCount > chunkSize){
            if(currentChunk.length > 0){
                ...
                chunks.push({
                    ...
                    pageNumber:0 //page number will be set during processing
                });
                currentChunk = [];
                currentWordCount = 0;
            }
            ...
            for(let i=0; i<paragraphWords.length; i+=chunkSize-overlap){
                const subChunk = paragraphWords.slice(i, i+chunkSize);
                chunks.push({
 ...
 * @returns {Array<Object>} Relevant chunks
 */
export const findRelevantChunks = (chunks,query, maxChunks=3)=>{
    ...
    if(queryWords.length === 0){
        //returns clean chunk objects without mongoose metadata
