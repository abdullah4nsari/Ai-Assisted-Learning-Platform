...
import {
    ...
    CheckCircle2, Loader2, ShieldCheck, Pencil, X, Sparkles,
} from 'lucide-react';
...
const InputField = ({ label, id, type = 'text', value, onChange, icon: Icon, disabled, placeholder, rightSlot, error }) => (
    <div className="space-y-1.5">
        <label htmlFor={id} className="block text-xs font-semibold text-slate-500 uppercase tracking-wider">{label}</label>
        <div className="relative">
            {Icon && (
                <Icon size={15} className={`absolute left-3.5 top-1/2 -translate-y-1/2 ${disabled ? 'text-slate-300' : 'text-slate-400'}`} />
            )}
            <input
                id={id} type={type} value={value} onChange={onChange} disabled={disabled} placeholder={placeholder}
                className={`w-full py-2.5 text-sm rounded-xl border outline-none transition-all duration-200
...
const Card = ({ icon: Icon, iconGradient, title, subtitle, children, delay = 0 }) => (
    <motion.div {...fadeUp(delay)} className="bg-white rounded-2xl border border-slate-100/80 shadow-sm p-6 space-y-5">
...
const PasswordInput = ({ label, id, value, onChange, placeholder, error }) => {
    ...
    return (
        <InputField
            ...
            value={value} onChange={onChange} icon={Lock} placeholder={placeholder} error={error}
            rightSlot={
                <button type="button" onClick={() => setShow(s => !s)} className="text-slate-400 hover:text-slate-600 transition-colors" tabIndex={-1}>
...
const Avatar = ({ name, profileImage }) => {
    ...
    if (profileImage) return <img src={profileImage} alt={name} className="w-20 h-20 rounded-2xl object-cover shadow-xl shadow-emerald-500/25 flex-shrink-0" />;
    return (
...
const ProfilePage = () => {
    ...
    const handleProfileSave = async () => {
        ...
        try {
            ...
            updateUser({ username: updated?.usernmae || updated?.username || username.trim(), email: updated?.email || email });
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6 pb-10">
            ...
            <motion.div
                {...fadeUp(0.05)}
                className="relative overflow-hidden bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-500 rounded-2xl p-5 flex items-center gap-4 shadow-xl shadow-emerald-500/25"
            >
                ...
                <div className="min-w-0 relative z-10">
                    <div className="flex items-center gap-1.5 mb-0.5">
                        ...
                        <p className="text-emerald-100 text-xs font-medium">Signed in as</p>
                    </div>
                    ...
                    {memberSince && <p className="text-emerald-200 text-xs mt-0.5">Member since {memberSince}</p>}
                </div>
            </motion.div>

            {/* User Info */}
            <Card icon={User} iconGradient="from-blue-400 to-cyan-500" title="User Information" subtitle="Update your display name" delay={0.1}>
                <div className="space-y-4">
                    <InputField
                        label="Username" id="username" value={username}
                        onChange={e => { setUsername(e.target.value); setProfileErrors(p => ({ ...p, username: '' })); }}
                        icon={User} placeholder="Your username" disabled={!editingProfile} error={profileErrors.username}
                    />
                    <InputField label="Email Address" id="email" type="email" value={email} icon={Mail} disabled placeholder="your@email.com" />
                </div>
                <div className="flex items-center gap-2 pt-1">
                    {!editingProfile ? (
                        ...
                        <>
                            ...
                            >
                                {profileLoading ? <><Loader2 size={14} className="animate-spin" /> Saving…</> : <><CheckCircle2 size={14} /> Save Changes</>}
                            </motion.button>
                            <button
                                onClick={() => { setUsername(user?.username || ''); setEmail(user?.email || ''); setProfileErrors({}); setEditingProfile(false); }}
            ...
            {/* Change Password */}
            <Card icon={ShieldCheck} iconGradient="from-violet-500 to-indigo-500" title="Change Password" subtitle="Use a strong password you don't use elsewhere" delay={0.15}>
                <form onSubmit={handlePasswordChange} className="space-y-4" noValidate>
                    <PasswordInput label="Current Password" id="currentPassword" value={currentPassword}
                        onChange={e => { setCurrentPassword(e.target.value); setPasswordErrors(p => ({ ...p, currentPassword: '' })); }}
                        placeholder="Enter current password" error={passwordErrors.currentPassword} />
                    <PasswordInput label="New Password" id="newPassword" value={newPassword}
                        onChange={e => { setNewPassword(e.target.value); setPasswordErrors(p => ({ ...p, newPassword: '' })); }}
                        placeholder="At least 6 characters" error={passwordErrors.newPassword} />
                    <PasswordInput label="Confirm New Password" id="confirmPassword" value={confirmPassword}
                        onChange={e => { setConfirmPassword(e.target.value); setPasswordErrors(p => ({ ...p, confirmPassword: '' })); }}
                        placeholder="Repeat new password" error={passwordErrors.confirmPassword} />
                    ...
                    >
                        {passwordLoading ? <><Loader2 size={14} className="animate-spin" /> Changing…</> : <><ShieldCheck size={14} /> Change Password</>}
                    </motion.button>
                </form>
            </Card>
        </div>
